...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderFormattedText(rawText) {
        ...
        const isMarkdown = trimmedText.startsWith('#');
        let htmlOutput = "";

        if (isMarkdown) {
            console.log("Markdown mode detected.");
            // Remove the leading '#' marker that enables the mode
            const mdText = trimmedText.substring(1);
            ...
            lines.forEach(line => {
                ...
                // Remove leading '#' and trim whitespace for content
                const content = trimmedLine.substring(level).trim();

                // Close the previous heading tag if it's open before starting new line/tag
                if (currentHeadingLevel > 0) {
                    htmlOutput += \`</h\${currentHeadingLevel}>\\n\`;
                    currentHeadingLevel = 0; // Mark as closed for this line iteration
                }

                if (level > 0 && content) { // It's a heading line with content
                    // Start a new heading tag
                    htmlOutput += \`<h\${level}>\${content}\`; // No newline here, closing tag adds it
                    currentHeadingLevel = level; // Mark this heading level as open
                } else if (content) { // Regular line with content
                    // Treat as a paragraph line, use <br> for line break within paragraphs
                    htmlOutput += content + '<br>\\n';
                } else if (!currentHeadingLevel) { // Handle empty lines between paragraphs/headings if no heading is open
                     htmlOutput += '<br>\\n';
                }
                // If line was only whitespace and a heading was open, the </h...> handles the break.
            });

            // Close any remaining open heading tag at the very end
            if (currentHeadingLevel > 0) {
                htmlOutput += \`</h\${currentHeadingLevel}>\\n\`;
            }
            ...
            // Consolidate potential multiple breaks from empty lines etc.
            htmlOutput = htmlOutput.replace(/(<br\\s*\\/?>\\s*)+/g, '<br><br>');
             // Remove breaks immediately after closing heading tag
             htmlOutput = htmlOutput.replace(/(<\\/h\\d>)\\s*<br\\s*\\/?>(?:\\s*<br\\s*\\/?>)*/gi, '$1');
             // Remove breaks immediately before opening heading tag
             htmlOutput = htmlOutput.replace(/<br\\s*\\/?>(?:\\s*<br\\s*\\/?>)*\\s*(<h\\d>)/gi, '$1');
